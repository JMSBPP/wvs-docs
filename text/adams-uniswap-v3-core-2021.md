...
1Hayden Adams, Noah Zinsmeister, Moody Salem, River Keefer, and Dan Robinson
...
3.2.1 Non-CompoundingFees. Feesearnedinearlierversionswere
...
another address.
5 ORACLE UPGRADES
Uniswapv3includesthreesignificantchangestothetime-weighted
...
Anotherchangeisthatinsteadofaccumulatingthesumofprices,
...
alongside the price accumulator, which accumulates 1
𝐿 for each
...
provision,asdescribedinsection6.Inaddition,theaccumulatorcan
bestoredinasmallernumberofbits,sinceittrackslog𝑃 ratherthan
𝑃, and log𝑃 can represent a wide range of prices with consistent
...
v3accumulatesthecumulativesumofthecurrenttickindex(𝑙𝑜𝑔1.0001𝑃,
...
In addition to the seconds-weighted accumulator of log1.0001 𝑝𝑟𝑖𝑐𝑒,
Uniswap v3 also tracks a seconds-weighted accumulator of 1
𝐿 (the
...
at an even rate of 𝑅 tokens per second to all active liquidity in the
...
movementsareusuallymodeled).ThearithmeticmeanofageometricBrownianmotion
...
4Uniswap v3 Core
contract, and a position with 𝐿 liquidity was active from 𝑡0 to 𝑡1,
...
adjacent).
Eachrangecanbespecifiedasapairofsignedintegertickindices:
...
power of 1.0001. Identifying ticks by an integer index𝑖, the price at
each is given by:
𝑝(𝑖) = 1.0001𝑖
(6.1)
...
Asanexample,
√
𝑝(0)—thesquarerootpriceattick0—is1,
√
𝑝(1)
is
√
...
blebytickSpacingcanbeinitialized.Forexample,iftickSpacing
is 2, then only even ticks (...-4, -2, 0, 2, 4...) can be initialized. Small
choicesfortickSpacingallowtighterandmorepreciseranges,but
...
the pool’s current reserves, 𝑥 and𝑦. In Uniswap v3, the contract
could be thought of as having virtual reserves—values for 𝑥 and𝑦
...
5Hayden Adams, Noah Zinsmeister, Moody Salem, River Keefer, and Dan Robinson
...
(𝑓𝑔,0)andfeeGrowthGlobal1(𝑓𝑔,1).Theserepresentthetotalamount
...
amountoffeesthatwouldhavebeenearnedby1unitofunbounded
...
protocolfeeineachtoken,protocolFees0(𝑓𝑝,0)andprotocolFees1
(𝑓𝑝,1). This is an unsigned uint128. The accumulated protocol fees
canbecollectedbyUNIgovernance,bycallingthecollectProtocol
function.
...
𝑦 + Δ𝑦
(6.12)
Butrememberthatinv3,thecontractactuallytracksliquidity(𝐿)
and square root of price (
√
𝑃) instead of 𝑥 and𝑦. We could compute
...
cumulative fees per share 𝑓𝑟 in the range between two ticks—a
...
as a tick𝑖 is crossed in either direction, its 𝑓𝑜 (for each token) should
be updated as follows:
𝑓𝑜 (𝑖) := 𝑓𝑔 − 𝑓𝑜 (𝑖) (6.20)
𝑓𝑜 is only needed for ticks that are used as either the lower or
upperboundforatleastoneposition.Asaresult,forefficiency, 𝑓𝑜 is
...
accounting,since,asdescribedbelow,allthepositionneedstoknow
is the growth in 𝑔 within a given range since that position was last
touched.
7Hayden Adams, Noah Zinsmeister, Moody Salem, River Keefer, and Dan Robinson
...
tickCumulativeOutside tracks the log1.0001 𝑃 accumulator de-
...
𝑡1 and 𝑡2 can be computed by recording the value of 𝑠𝑟 (𝑖𝑙,𝑖𝑢) at 𝑡1
...
ically, liquidity could be thought of as
√
𝑥 ·𝑦, where 𝑥 and𝑦 are
...
TwooftheargumentstosetPosition—lowerTickandupperTick—
...
liquidityDelta.ItalsoaddsliquidityDeltatotheliquidityNet
