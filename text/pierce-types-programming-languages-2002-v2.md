---
sha256: e31213ed6e4b55a542820e93c13119820a82af1678969923f6decb6a1e246b97
...
corrections are welcome.Contents
Preface 8
## 1 Introduction 13
...
2 MathematicalPreliminaries 18
...
14.3 Is the Lambda-Calculus a Programming Language? . . . . . . . . . . 39
## 4.4 Formalities . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 39
Syntax . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 39
Substitution . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 40
...
7.6 Further Reading . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 668 Extensions 67
## 8.1 Base Types . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 67
## 8.2 Unit type . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 67
## 8.3 Let bindings . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 68
...
14.6 Classes with “Self” . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 10615 Recursive Types 109
## 15.1 Examples . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 109
Lists . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 109
...
ML Code . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 141
18.7 Further Reading . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 14319 ExistentialTypes 144
## 19.1 Motivation . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 144
...
Interpreting Recursive Types . . . . . . . . . . . . . . . . . . . . . . 20122.4 Further Reading . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 203
...
Earlychapters on the untyped lambda-calculushelp make the book self-contained,
...
begins with programmingexamplesmotivating anew typing feature,developsthe
...
almost entirely operational, for a close correspondencewith familiarprogramming
languages.
Arepeatedtheme isthetheoreticalpropertiesrequiredtobuildsound andcom-
...
8January 15, 2000 PREFACE 9
researchers in other areas who want to learn something about type systems, and
should be a useful referencefor experts. The first several chaptersshould be usable
...
languages and compilers (abstract syntax, Backus-Naurgrammars, evaluation, ab-
stract machines, etc.). This material is available in many excellent undergraduate
texts. I particularly like the one by Friedman, Wand, and Haynes [FWH92].January 15, 2000 PREFACE 10
Course Outlines
...
 basictheory andimplementation (omitting orskimming the endof thebook).
...
chanicallycheckedduringtypesetting: anautomaticscriptgoesthrougheachchap-
...
the text is available at http://www.cis.upenn.edu/~bcpierce/typesbook. TheseJanuary 15, 2000 PREFACE 11
2
3
4
6
...
26
25
Figure 1: Chapter dependenciesJanuary 15, 2000 PREFACE 12
...
plementation exercisesand largercourse projects. The implementation language is
the ObjectiveCamldialectofML,freelyavailablethrough http://caml.inria.fr.
...
Many!Chapter 1
Introduction
Proofs of programsare tooboring for thesocial process of mathematicsto work.
...
denotationalsemantics; these canbeusedto expressverygeneralcorrectnessprop-
...
framework itself. Atthe other end aretechniques of much more limited power—so
...
Typesystems aregenerallyformulatedascollections ofrules forchecking the“con-
...
1980s bounded quantification [CW85, CG92, CMMS94]January 15, 2000 1. INTRODUCTION 15
...
1990s typed intermediate languages [TMC+
96]
...
guishing betweennatural-number-valuedvariablesandarithmeticexpressionsand
...
languages, the specificationlanguagesused in program verification, and the
...
it goes down very well, but to put it all in writing will require a bit of research.January 15, 2000 1. INTRODUCTION 16
...
 In“self-certification”ofuntrustedcode(so-called“proof-carryingcode”[NL96,Nec97,
NL98])
 In security
 In theorem proving
 In databases
...
lambda calculus, a range of type systems, and many aspects of semantics.January 15, 2000 1. INTRODUCTION 17
...
Mitchell or Abadi and Cardelli.Chapter 2
...
for each s : S, that (8s0 < s: P(s0)) implies P(s), then we may conclude that P(s)
...
tionary ordering” on pairs of natural numbers: (m;n) < (m0;n0) iff m < m0 or
m = m0 and n < n0.
Now, suppose that P is some predicate on pairs of natural numbers. If we can
show, for each (m;n), that (8(m0;n0) < (m;n): P(m0;n0)) implies P(m;n), then
...
I 1
iszero (pred (succ 0));
20January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 21
I true
iszero (succ (succ 0));
I false
...
For brevity, the examples use standardarabic numerals as shorthand for nested
...
1. ftrue; false; 0g T;
2. if t1 2T, then fsucc t1; pred t1; iszero t1g T;
3. if t1 2T, t2 2T, and t3 2T, then if t1 then t2 else t3 2T.
These three clauses capture exactly what is meant by the productions in the more
concise and readable “abstract grammar” notation that we used above. 2January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 22
...
S0 = ;
Si+1 = ftrue; false; 0g
[ fsucc t1; pred t1; iszero t1 jt1 2Sig
[ fif t1 then t2 else t3 j t1;t2;t3 2Sig:
Finally, let
S =
[
i
...
conditions has Sas a subset (i.e., that Sis the smallest set satisfying the conditions).
For part (a), we must check that each of the three conditions in Definition 3.2.1
holds of S. First, since S1 = ftrue; false; 0g and S1  S
...
similar argument.
For part (b), suppose that some set S0 satisfies the three conditions in Defini-
tion 3.2.1. We will argue, by induction on n, that every Sn  S0, from which itJanuary 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 23
will clearly follow that S  S0. Suppose that Sm  S0 for all m < n; we must
then show that Sn S0. Since the definition of Sn has two clauses (for n = 0 and
n = i+ 1), there are two cases to consider.
 If n = 0, then Sn = ;. But ;S0 trivially.
...
1. t is a constant, hence t 2S0 by condition (1).
2. t has the form succ t1, pred t1, or iszero t1, for some t1 2 Si. But
then, by the induction hypothesis, t1 2 S0, and so, by condition (2),
t 2S0.
3. thas the form if t1 then t2 else t3, for some t1;t2;t3 2Si. Again,
by the induction hypothesis, t1, t2, and t3 are all in S0, and hence, by
condition (3), so is t.
Thus, we have shown that each Si  S0. By the definition of S as the union of all
the Si, this gives S S0, completing the argument. 2
The explicit characterization of T justifies an important principle for reasoning
about its elements. If t 2T, then one of three things must be true about t—either
1. t is a constant, or
2. t has the form succ t1, pred t1, or iszero t1 for some smaller term t1, or
3. thas the form if t1 then t2 else t3 for some smaller terms t1, t2, and t3.
...
Consts(if t1 then t2 else t3) = Consts(t1) [Consts(t1) [Consts(t1) 2January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 24
...
Equivalently, depth(t), is the smallest i such that t 2Si. 2
...
jConsts(t)j = jConsts(t1)j size(t1) < size(t).January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 25
...
pred t1 !pred t0
1
(E-PRED)January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 26
...
in these cases, such as dumping core.January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 27
## 3.3 Properties
...
3.3.2 Proposition [Determinacy of evaluation]: If t ! t0 and t ! t00, then
t0 = t00. 2
...
themathematicaldefinitions, not for efficient execution! We’ll ignore parsingand printing,
...
printing functions.January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 28
Evaluation
exception No
let rec eval1 t =
match t with
...
pred t1 !pred t0
1
(E-PRED)January 15, 2000 3. UNTYPED ARITHMETIC EXPRESSIONS 30
iszero 0 !true (E-BETANATIZ)
iszero (succ v) !false (E-BETANATIS)
t1 !t0
1
...
lambda-calculushas seen widespreaduse in the specification of programming lan-
...
Abadi and LucaCardelli’s objectcalculus[AC96] distills the core featuresof many
...
## 4.1 Basics
Proceduralabstraction is a key featureof most programming languages. Instead of
...
Inthe1930s,AlonzoChurchinventedamathematicalsystem calledthelambda-
...
returned by a function is another function.January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 33
Syntax
...
sense thatthey standfor termsof the lambda-calculus,and“meta”in thesense that
...
issues of lexing, parsing, operatorprecedence,etc. For presentpurposes, though, it
is better to avoid such issues by concentrating on the abstract syntax, using paren-
theses informally asnecessary to clarifythetreestructureof examples. Application
...
times called combinators. The simplest combinator is called the identityfunction:
id = x.x;January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 34
...
which iscapturedinthefollowing evaluationrule,traditionallycalledbeta-reduction.
...
The lambda-calculusis much more powerful than its tiny definition might suggest.
...
b = fls.
test = l. m. n. l m n;January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 35
...
fls, so the first and second projection functions fst and snd can be implementedJanuary 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 36
...
nis representedby a function that does something n times—a kind of active unary
numeral.
We can define some common arithmetic operations on Church numerals as fol-
lows:
plus = m. n. s. z. m s (n s z);
times = m. n. m (plus n) c0;
Here, plusis a combinator that takes two Church numerals, mand n, as arguments,
...
time, applying it to just one argument n yields the function that adds nto whatever
...
4.2.3 Exercise [Recommended]: Define a similar term forcalculating the successor
of a number. (Solution on page 254.) 2January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 37
...
can be done using the following rather tricky “predecessorfunction,” which, given
...
as its argument and yields (pair cj cj+1) as its result. So applying ss m times to
...
Canyou think of any differentways of encoding lists? (Solution on page254.)2
...
factorialwould intuitively be:
if n=0 then 1
else n * (if n-1=0 then 1
else (n-1) * (if (n-2)=0 then 1
else (n-2) * ...))January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 39
This effectcan be achievedusing fixpointby defining gg = f.hbody containing fi
...
(Solution on page 254.) 2
4.3 IstheLambda-CalculusaProgrammingLanguage?
## 4.4 Formalities
...
1. x 2T for every x 2V;
2. if t1 2T and x is a variable name, then x.t1 2T;
3. if t1 2T and t2 2T, then (t1 t2) 2T. 2
...
the set of variables appearing “free” in a lambda-term.January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 40
...
fx 7!sg(t1 t2) = fx 7!sgt1 fx 7!sgt2January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 41
...
fx 7!sg(y.t1) =
y. t1 ify = x
y. fx 7!sgt1 ify 6= xandy = 2FV(s)
fx 7!sg(t1 t2) = fx 7!sgt1 fx 7!sgt2January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 42
...
fx 7!sgx = s
fx 7!sgy = y if y 6= x
fx 7!sg(y.t1) = y. fx 7!sgt1 if y 6= x and y = 2FV(s)
...
of bound variables? 2January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 43
Operational Semantics
...
v1 t2 !v1 t0
2
(E-APP2)January 15, 2000 4. THE UNTYPED LAMBDA-CALCULUS 44
## 4.5 Further Reading
Information on the untyped lambda-calculus can be found in many places. The
firstandbestisBarendregt’sencyclopedicmonograph, TheLambdaCalculus[Bar84].
...
PJL92]) and programming language semantics (e.g.[Sch86, Gun92, Win93, Mit96]).Chapter 5
Implementing the
Lambda-Calculus
...
sures.
Wechoose thesecond, using awell-known technique duetoNicolasdeBruijn[dB72].
...
Actually,allweneedto know about is theorderin which the variablesappear.
Instead of writing it in tabular form, as we did above, we will elide the numbers
and write it as a sequence.January 15, 2000 5. IMPLEMENTING THE LAMBDA-CALCULUS 47
...
that
1. k 2Tn whenever 0 k < n;
2. if t 2Tn and n > 0, then .t 2Tn-1;
3. if t1 2Tn and t2 2Tn, then t1 t2 2Tn.
...
context and producesan ordinaryterm. (To do this, you will need to “make
up”names forthe variablesbound by abstractions in t. You may assume that
the set V of variable names is ordered, so that it makes sense to say “choose
the smallest variable name that is not already in dom( ).”)January 15, 2000 5. IMPLEMENTING THE LAMBDA-CALCULUS 48
Thispairoffunctions should havethepropertythatremovenames (restorenames (t)) =
...
## 5.1.6 Definition [Shifting]: The d-place shift of a term t above cutoff c, written
"d
...
c (t1) "d
c (t2)
We will write "d(t) for "d
0 (t). 2
## 5.1.7 Exercise [Quick check]:
1. What is "2(.. 1 (0 2))?
2. What is "2(. 0 1 (. 0 1 2))? 2
5.1.8 Exercise: Show that if t is an n-term, then "d
c (t) is an (n+d)-term. 2
Now we are ready to define the substitution operator fj 7! sgt. At the end, we
will be most interested in substituting for the last variablein the context (i.e., j = 0),
...
fj 7!sg(t1 t2) = fj 7!sgt1 fj 7!sgt2 2January 15, 2000 5. IMPLEMENTING THE LAMBDA-CALCULUS 49
...
5.1.13 Exercise [Recommended]: Thedefinition ofsubstitution on namelessterms
...
let tmsubstsnip s t = tmshift (tmsubst (tmshift s 1) t) (-1)January 15, 2000 5. IMPLEMENTING THE LAMBDA-CALCULUS 51
Evaluation
...
1. wealwayswork“uptoalpha-conversion,”ignoring theparticularnames
chosen for bound variables, and
2. whenever we mention a term, we will always (at least implicitly) have
some context in mind that imposes a numerical ordering on its freevari-
able names.Chapter 6
...
succ t1 : Nat
(T-SUCC)
52January 15, 2000 6. TYPED ARITHMETIC EXPRESSIONS 53
...
stances of the remaining rules. (In the literature on type systems, the symbol 2is
...
with conclusion `t : T. We sometimes write D :: J to indicate that Dis a deriva-
...
D :: J, that (P(J0), for all D0 :: J0 with size(D0) < size(D)) implies P(J), then we
may conclude that P(J) holds for every derivable typing statement. 2January 15, 2000 6. TYPED ARITHMETIC EXPRESSIONS 54
Typechecking
...
t0, then `t0 : T. 2
Proof: Exercise. 2January 15, 2000 6. TYPED ARITHMETIC EXPRESSIONS 55
...
but not a value.
6.3.7 Theorem [Progress]: If ` t : T, then either t is a value or there is some t0
such that t !t0. 2
Proof: Exercise. 2
...
always the case that, if t ! t0 and ` t0 : T, then `t : T? If so, prove it. If not,
...
let tyS = typeof s2 inJanuary 15, 2000 6. TYPED ARITHMETIC EXPRESSIONS 56
...
Nat type of natural numbers
New typing rules (t : T)
0 : Nat (T-ZERO)January 15, 2000 6. TYPED ARITHMETIC EXPRESSIONS 57
t1 : Nat
succ t1 : Nat
(T-SUCC)
t1 : Nat
...
7.1.2 Definition: Theabstractsyntaxofsimplytypedlambda-terms(withbooleans
...
7.2.3 Exercise [Quick check]: Which of the rules for the type Boolareintroduction
rules and which are elimination rules? (Solution on page 255.) 2January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 61
## 7.3 Summary
...
booleans. We thereforesplit the formal summary of the system into two pieces: the
...
; x:T1 `t2 : T2
`x:T1.t2 : T1!T2
(T-ABS)January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 62
`t1 : T11!T12 `t2 : T11
`t1 t2 : T12
(T-APP)
...
implementation in Section 7.5. 2January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 63
...
of which is that well-typed terms arein one-to-one correspondencewith the typing
derivations that justify their well-typedness (in a given environment). The typing
derivation can be recoveredimmediately from the term, and vice versa. In fact, the
...
prove it. 2January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 64
...
text , and that r is some subterm of s. We write typeOf (r) for the type R such
...
s = f:A!B. g:A. f gand r = f g, then typeOf (r) = B. When (as in this chap-
...
Type Soundness
7.4.11 Theorem [Preservation of types during evaluation]: If `t : Tandt !
t0, then `t0 : T. 2
Proof: 2
...
false and every closed value of type S!T is a  abstraction. 2January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 65
## 7.5 Implementation
type ty =
TyArr of ty * ty
| TyBool
...
else error fi "guard of conditional not a boolean"January 15, 2000 7. SIMPLY TYPED LAMBDA-CALCULUS 66
## 7.6 Further Reading
...
8.4 Records and TuplesJanuary 15, 2000 8. EXTENSIONS 69
...
lambda calculus with records by adding a new syntactic category of patterns, plusJanuary 15, 2000 8. EXTENSIONS 70
...
from the pure untyped lambda-calculus with records and ordinary let-binding:January 15, 2000 8. EXTENSIONS 71
...
def
= {...i=pi...}
Your job is to addtypes to this calculus, in the style of the simply typedlambda-
calculus:
...
correct order.)
(Solution on page 256.) 2January 15, 2000 8. EXTENSIONS 72
## 8.5 Variants
Variants ! <>
New syntactic forms
t ::= ... (terms...)
...
8.6 General recursionJanuary 15, 2000 8. EXTENSIONS 73
...
New evaluation rules (t !t0)January 15, 2000 8. EXTENSIONS 74
t1 !t0
1
...
`t1 : List T1
`tail t1 : List T1
(T-TAIL)January 15, 2000 8. EXTENSIONS 75
...
(These are used only in the advanced parts of the object examples.)Chapter 9
References
...
; empty context
;x:T term variable binding
76January 15, 2000 9. REFERENCES 77
 ::= ; empty store
; l = t location binding
 ::= ; empty store typing
; l:T location typing
Evaluation (tj  !t0 j 0 )
...
; x:T1 j  `t2 : T2
j  `x:T1.t2 : T1!T2
(T-ABS)January 15, 2000 9. REFERENCES 78
...
in turn, is established by an induction on a derivation of t j  ! t0 j 0. All
...
ing on well-typed terms? (Solution on page 260.) 2January 15, 2000 9. REFERENCES 79
## 9.1 Further Reading
Thepresentationofreferencesin thischapterisadaptedfromHarper’streatment[?].
...
JG91, TJ92, TJ92, LW91, Wri92] and the references cited there.Chapter 10
Exceptions
...
`t : T
(T-EQ)
81January 15, 2000 11. TYPE EQUIVALENCE 82
...
`S T
(Q-TRANS)
83January 15, 2000 12. DEFINITIONS 84
`S1 T1 `S2 T2
`S1!S2 T1!T2
(Q-ARROW)
X$T 2
...
;x:T1 ` I t2 : T2
` I x:T1.t2 : T1!T2
(TA-ABS)January 15, 2000 12. DEFINITIONS 85
...
rule” for this reason.January 15, 2000 13. SUBTYPING 88
13.1.1 Exercise [Quick check]: Demonstratethatthepremisesof S-ARROW areboth
necessarybygiving ill-behavedprogramsthatwouldbewelltypedifeitherpremise
were dropped. 2
...
3. T[ ] = {x:Top,y:[ ]}!Nat!Nat
4. T[ ] = [ ]!Top!Top
5. T[ ] = ([ ]!Top)!TopJanuary 15, 2000 13. SUBTYPING 89
6. T[ ] = [ ]!Nat![ ]
7. T[ ] = [ ]
8. T[ ] = Nat 2
Summary
...
S <: U U <: T
S <: T
(S-TRANS)January 15, 2000 13. SUBTYPING 90
S <: Top (S-TOP)
T1 <: S1 S2 <: T2
S1!S2 <: T1!T2
(S-ARROW)
...
i21::n} <: {li:Ti
i21::n}
(S-RCD-DEPTH)January 15, 2000 13. SUBTYPING 91
...
2. IfS <: Tcanbederivedatall,thenitcanbederivedwithout using S-TRANS.2
Proof: Exercise. 2
...
under the following rules:January 15, 2000 13. SUBTYPING 92
...
typerelation(andhencealsothedeclarativesubtyperelation,byProposition 13.2.5).
...
` I t : T. This is called the algorithmic typing relation.January 15, 2000 13. SUBTYPING 93
2. Show that, although they aredefined differently,the ordinary typing relation
...
S2 <: T2
(T-SUB)
`t1 t2 : T2January 15, 2000 13. SUBTYPING 94
...
harmedby moving most uses of subsumption as lateaspossible in atyping deriva-
...
` I t1 : T1 T1 = T11!T12 ` I t2 : T2 ` I T2 <: T11
` I t1 t2 : T12
(TA-APP)January 15, 2000 13. SUBTYPING 95
for each i ` I ti : Ti
` I {l1=t1...ln=tn} : {l1:T1...ln:Tn}
(TA-RCD)
` I t : {l1:T1...ln:Tn}
...
naryderivation canalmostbe provedbyan algorithmic derivation.
...
two incomparable types. 2January 15, 2000 13. SUBTYPING 96
...
| _ ! error fi "arrow type expected")January 15, 2000 13. SUBTYPING 97
| TmRecord(fi, fields) !
...
in let f = loop (f1,f2)January 15, 2000 13. SUBTYPING 98
in (match f with
...
(fun () ! prtm ctx t)
(fun tyT ! prty tyT)
elseJanuary 15, 2000 13. SUBTYPING 99
typeof' ctx t
13.4.1 Exercise [Recommended]: Addbooleansandconditionals toboth thedeclar-
...
13.4.2 Exercise [Recommended]: The directory simpleplusin the course web site
...
program). An object’s methods areinvoked by the operation of message-sending.
...
We maywant to createandmanipulate alot of counters, so it will beconvenient
...
same as (x:T.x)t):January 15, 2000 14. IMPERATIVE OBJECTS 102
c =
let r = ref 0 in
{get = _:Unit. !r,
set = i:Nat. r:=i,
...
creates a new counter every time it is called.January 15, 2000 14. IMPERATIVE OBJECTS 103
newCounter =
_:Unit.
(let r = {x=ref 0} in
{get = _:Unit. !(r.x),
...
Note that our old inc3function on counters can safely use our refined counters
too:
rc = newResetCounter unit;
(inc3 rc; rc.reset unit; inc3 rc; rc.get unit);
I rc : ResetCounter
3 : NatJanuary 15, 2000 14. IMPERATIVE OBJECTS 104
## 14.4 Basic classes
...
Hereisone verysimplewaytodefinenewResetCounterintermsofnewCounter:
newResetCounter =
_:Unit.
...
inc methods (no set), then we couldn’t write newResetCounterin this way.
...
Now we canbuild aclassfor ResetCounterby re-usingthe fields ofcounterClass:
resetCounterClass =
r:CounterRep.
...
Thedefinitions ofnewCounterandnewResetCounterbecometrivial: wesimply
allocatetheinstancevariablesandsupplythemto counterClassorresetCounterClass,
where the real work happens:January 15, 2000 14. IMPERATIVE OBJECTS 105
newCounter =
_:Unit.
(let r = {x=ref 0} in
counterClass r)
...
({get = super.get,January 15, 2000 14. IMPERATIVE OBJECTS 106
set = super.set,
inc = super.inc,
reset = _:Unit. r.x:=!(r.b),
backup = _:Unit. r.b:=!(r.x)}
as BackupCounter);
I backupCounterClass : BackupCounterRep ! BackupCounter
The variable superwas used above to “copy functionality” from the superclass
...
{get = _:Unit. !(r.x),
set = i:Nat. r.x:=i,
inc = _:Unit. self.set (succ(self.get unit))});January 15, 2000 14. IMPERATIVE OBJECTS 107
...
newclassbysubclassingcounterClass, wecanoverridethemethodsofcounterClass
in such a way that recursive calls to these methods from within counterClasswill
...
InstrumentedCounter ! InstrumentedCounterJanuary 15, 2000 14. IMPERATIVE OBJECTS 108
...
Note how the accessesmethod counts calls to both get and inc:
...
ic.accesses unit;
I 3 : Nat
14.6.2 Exercise [Recommended]: Use the fullsubeffchecker from the course di-
...
(as in Section 14.5). 2Chapter 15
Recursive Types
...
(lam (x:D. ap f (ap x x))));
I fixD : D ! DJanuary 15, 2000 15. RECURSIVE TYPES 111
...
zro = <nat=0> as DBody;
I suc : D ! DBody
zro : DBodyJanuary 15, 2000 15. RECURSIVE TYPES 112
...
intuition that a recursive type is “the same as” its unfoldingJanuary 15, 2000 15. RECURSIVE TYPES 113
...
x:T 2
` I x : T
(TA-VAR)January 15, 2000 15. RECURSIVE TYPES 114
;x:T1 ` I t2 : T2
` I x:T1.t2 : T1!T2
(TA-ABS)
` I t1 : T1 ` I T1 *T11!T12 ` I t2 : T2 ` I T2 T11
...
tyeqv [] ctx tyS tyTJanuary 15, 2000 15. RECURSIVE TYPES 115
Typing
let rec typeof ctx t =
match t with
TmVar(fi,i,_) !
...
New evaluation rules (t !t0)January 15, 2000 15. RECURSIVE TYPES 116
...
(T-FOLD)
U = X.T1 `t1 : U
`unfold [U] t1 : fX 7!UgT1
(T-UNFOLD)
15.3.1 Exercise: Re-dotheexamplesin Section15.1usingtheUsingthefullisorec
...
during type substitution, because there areno binders for type variables. (This will
change when we discuss System F in Chapter 18; we will then have to treat type
substitutions more carefully.)
Substitution is extendedto contexts by defining ( )(x) to be ( (x)). Similarly,
a type substitution is applied to a term t by applying it to all types appearing in t.
If  and aretype substitutions, we write Æ for the type substitution formed
...
:
( (X)) if X 2dom( )
(X) if X = 2dom( ) and X 2dom()
undefined if X = 2dom( ) [dom():
Note that ( Æ )T = ( T). 2
...
that  `t : T for some T?
Accordingto the first view, type variablesshould beheldabstract during typecheck-
ing, thus ensuring that a well-typed term will behave properly no matter what
concrete types are later substituted for its type variables. For example, the termJanuary 15, 2000 17. TYPE RECONSTRUCTION 120
f:X!X. a:X. f (f a);
...
f:Y. a:X. f (f a);
is not typeable as it stands, but if we replaceY by Nat!Natand Xby Nat, we obtain
...
generalinstance of f:Y. a:X. f (f a), in the sense that it makes the least com-
...
(fX 7!Y!Zg; Z)
are all typings for ( ;t). 2January 15, 2000 17. TYPE RECONSTRUCTION 121
...
be performed later.
17.3.1 Definition: A constraint set C is a set of equations fSi = Ti
i21::ng. A substi-
tution is saidto satisfytheconstraint set Cif, foreachi, the substitution instances
...
C0 = C1 [C2 [C3 [fT1 = Nat; T2 = T3!T3g
`iter T t1 t2 t3 : T3 jX1[X2[X3 C0 (CT-ITER)
As usual, these rules (when read from bottom to top) determine a straightforward
procedure that, given and t, calculates T and C (and X) such that `t : T jX C.January 15, 2000 17. TYPE RECONSTRUCTION 122
...
17.3.3 Exercise [Quick check]: Construct aconstrainttyping derivationwhose con-
clusion is
...
 ;x:T1 `t1 : S2:
By T-ABS,  `x:T1.t1 : T1!S2 = (T1!S2) = T, as required.January 15, 2000 17. TYPE RECONSTRUCTION 123
Case CT-APP: t = t1 t2
S = X
`t1 : S1 j C1
`t2 : S2 j C2
...
S = X
X = X1 [X2 [fXg
C = C1 [C2 [fS1 = S2!XgJanuary 15, 2000 17. TYPE RECONSTRUCTION 124
...
offreshvariablenamesactuallyoccurs(beyondstipulatingthattheymust be“fresh
...
`F t : T jF0 C;
where , F, and t are inputs to the algorithm and T, F0, and C are outputs. When-
ever it needs a fresh type variable, the algorithm takes off the front element of F
and returns the rest of F as F0.January 15, 2000 17. TYPE RECONSTRUCTION 125
...
page 256.) 2
17.3.10 Exercise [Recommended]: Implement thealgorithmdiscussedinExercise17.3.9
...
fresh variablenames. There arelots of ways of doing this; here is a fairly direct one
...
## 17.4 Unification
We have given two differentcharacterizations of the set of typings for a given term
in a given context. But we have not addressedthe algorithmic problem of deciding
...
sets.January 15, 2000 17. TYPE RECONSTRUCTION 126
...
0, written  v0, if 0 = Æ for some substitution . 2
...
unify(C) = if C = ;, then fg
else let fS = Tg[C0 = C in
if S = T
then unify(C0)
else if S = X and X = 2FV(T)
then unify(fX 7!TgC0) ÆfX 7!Tg
else if T = X and X = 2FV(S)
then unify(fX 7!SgC0) ÆfX 7!Sg
else if S = S1!S2 and T = T1!T2
then unify(C0 [fS1 = T1; S2 = T2g)
else
fail
...
More formally:
1. unify(C) halts, either by failing or by returning a substitution, for all C;January 15, 2000 17. TYPE RECONSTRUCTION 127
2. if unify(C) = , then  is a unifier for C;
3. if Æ is a unifier for C, then unify(C) =  with  vÆ. 2
...
ables, which depend on the observation that, if unifies fX 7!TgC0, then ÆfX 7!Tg
unifies fX = Tg[C0.
Part (3) again proceeds by induction on the number of recursive calls in the
computation of unify(C). If C is empty, then unify(C) immediately returns fg; since
Æ = Æ Æfg, we have fg v Æ as required. If C is non-empty, then unify(C) chooses
some pair (S;T) from C and continues by cases on the shapes of S and T.
Case: S = T
Since Æ is a unifier for C, it also unifies C0. By the induction hypothesis, unify(C) =
 with  vÆ, as required.
Case: S = X and X = 2T
Since Æ unifies S and T, we have Æ(X) = Æ(T). So, for any type U, we have Æ(U) =
Æ(fX 7! TgU); in particular, since Æ satisfies C0 it must also satisfy fX 7! TgC0. The
induction hypothesis then tells us that unify(fX 7! TgC0) = 0, with Æ = Æ0
for some . Since unify(C) = 0 ÆfX 7! Tg, showing that Æ = Æ(0 ÆfX 7! Tg)
will complete the argument. Choose any type variable Y. If Y 6= X, then clearly
( Æ(0 ÆfX 7! Tg))Y = ( Æ0)Y = ÆY. On the other hand, ( Æ(0 ÆfX 7! Tg))X =
( Æ0)T = ÆX, as we saw above. Combining these observations, we see that ÆY =
( Æ(0 ÆfX 7!Tg))Y for all variables Y, i.e. Æ = ( Æ(0 ÆfX 7!Tg)).
Case: T = X and X = 2S
Similar.
Case: S = S1!S2 and T = T1!T2
Straightforward. Just note that Æ is a unifier of fS1!S2 = T1!T2g [C0 iff it is a
unifier of C0 [fS1 = T1; S2 = T2g.
...
whenever (0;T0) is also a typing for ( ;t;S;C), we have  v0. 2
...
checker that calculates principal typings, using the simplechecker provided in the
course directory as a starting point. 2January 15, 2000 17. TYPE RECONSTRUCTION 129
...
aprincipaltyping ateachstep. The factthatthe typings arealwaysprincipalis what
...
fora programming language. Although we canenrich it with numerous additional
...
130January 15, 2000 18. UNIVERSAL TYPES 131
...
We have seen this phenomenon alreadyin Chapter ??, and it will be the
...
tively “forget” information about the term’s behavior.January 15, 2000 18. UNIVERSAL TYPES 132
...
metricpolymorphism andsimplearithmetic overloading, butnot subtyping, while
...
stractingtypesoutoftermsandfilling them inlater—wemight aswelluselambda-
...
types like Nat!Nat for classifying ordinary functions like x:Nat.x, we now needJanuary 15, 2000 18. UNIVERSAL TYPES 133
...
18.3.1 Exercise [Quick check]: InExercise7.3.1,wesawthatthepuresimply typed
lambda calculus (with no base types or type variables) is actually a trivial system
containing no typeable terms. What about pure System F? 2January 15, 2000 18. UNIVERSAL TYPES 135
## 18.4 Examples
...
ofsimple algebraicdatatypeslikebooleans, numbers, andlists thatwesaw in
...
doubleNatArrowNat = double [Nat!Nat];January 15, 2000 18. UNIVERSAL TYPES 136
I doubleNat : (Nat!Nat) ! Nat ! Nat
doubleNatArrowNat : ((Nat!Nat)!Nat!Nat) ! (Nat!Nat) ! Nat ! Nat
Once instantiated with a type argument, doublecan be further applied to an actual
...
becomes typeableifwe make xapolymorphic function andaddatypeapplication:
...
18.4.1 Exercise [Quick check]: Usingthetyping rulesabove,convinceyourselfthat
...
We can write common boolean operations like not either in terms of cifJanuary 15, 2000 18. UNIVERSAL TYPES 137
...
cplus = m:CNat. n:CNat.
m [CNat] csucc n;
I cplus : CNat ! CNat ! CNatJanuary 15, 2000 18. UNIVERSAL TYPES 138
Or, more directly:
cplus = m:CNat. n:CNat.
X. s:X!X. z:X.
m [X] s (n [X] s z);
...
represent pairs of numbers? Write functionsJanuary 15, 2000 18. UNIVERSAL TYPES 139
pairNat : CNat!CNat!PairNat;
fstNat : PairNat!CNat;
sndNat : PairNat!CNat;
forconstructing elements ofthis typefrom pairsofnumbers andforaccessing their
...
function on churchnumerals. LetKstandforthe untypedlambda-termx. y. x
...
18.5.1 Theorem [Strong normalization]: Allwell-typedSystemFtermsarestrongly
normalizing. 2January 15, 2000 18. UNIVERSAL TYPES 140
...
for some X, S1, and S2.January 15, 2000 18. UNIVERSAL TYPES 141
6. Define the leftmost leaf of a type T as follows:
leftmost-leaf(X) = X
leftmost-leaf(S!T) = leftmost-leaf(S)
leftmost-leaf(8X.S) = leftmost-leaf(S):
ShowthatiffX1X2 7!Ag(8Y.T1) = fX1 7!Bg(8Z.(8Y.T1)!T2), thenleftmost-leaf(T1) =
Xi for some Xi 2X1X2.
7. Show that is not typeable in System F.
(Solution on page ??.) 2
...
erase(X. t) = erase(t)
erase(t [S]) = erase(t)
A term M in the untyped lambda-calculus is said to be typeablein System F if there
...
tymap
(fun c x n ! if x>=c thenJanuary 15, 2000 18. UNIVERSAL TYPES 142
...
j
t
let tytmsubst tyS t = tytmsubsti tyS 0 tJanuary 15, 2000 18. UNIVERSAL TYPES 143
...
type S.
144January 15, 2000 19. EXISTENTIAL TYPES 145
...
one (hidden) type component and one term component.1
The type S is often called
...
{9X, {a:X, f:X!X}}:
That is, the right-hand component of p is a recordcontaining a field a of type X and
a field f of type X!X, for some X (namely Nat).
The same package p also has the type
{9X, {a:X, f:X!Nat}};
since its right-hand component is a record with fields aand f of type Xand X!Nat,
...
{9X1=S1, 9X2=S2, t1, t2}
def
= {9X1=S1, {9X2=S2, {t1, t2}}}January 15, 2000 19. EXISTENTIAL TYPES 146
...
computing t2.
For example, suppose that p has the following existential type:January 15, 2000 19. EXISTENTIAL TYPES 147
I p : {9X, {a:X,f:X!Nat}}
...
1. Checkthesubexpressiont1 andensurethatithasanexistentialtype{9X.T11}.
...
containing freevariableswithnegativeindices; typechecking mustfailatthispoint.
let {X,x}=p in x.a;
I Error: Scoping error!January 15, 2000 19. EXISTENTIAL TYPES 148
...
Interms of theanalogy with modules, this rule canbeviewedas akind of“link-
...
`t2 : fX 7!UgT2
`{9X=U,t2} as {9X,T2} : {9X,T2}
(T-PACK)January 15, 2000 19. EXISTENTIAL TYPES 149
`t1 : 9X.T12 ;X; x:T12 `t2 : T2
`let {X,x}=t1 in t2 : T2
(T-UNPACK)
...
threeoperationson counters: aconstant new, afunction getforextractingacounter’s
...
declaration, such as might be found in a program in Ada [oD80] or Clu [LAB+
81]:January 15, 2000 19. EXISTENTIAL TYPES 150
ADT counter =
type Counter
representation Nat
operations
...
tially identical.
Notethatwecansubstitute analternativeimplementation oftheCounterADT—
...
in
counter.get (addthree counter.new);January 15, 2000 19. EXISTENTIAL TYPES 151
I 3 : Nat
...
isempty. Use the Listtype introduced in Exercise?? as the underlying representa-
tion. Write a simple main program that createsa stack, pushes a couple of numbers
onto it, pops off the top element, and returns it.
This exercise is best done on-line. Use the checker named “everything” and
copy the contents of the file test.ffrom the everythingdirectory (which contains
definitions of the List constructor and associated operations) to the top of your
own input file. 2January 15, 2000 19. EXISTENTIAL TYPES 152
Existential Objects
...
sendget = c:Counter.
let {X,body} = c in
body.methods.get(body.state);January 15, 2000 19. EXISTENTIAL TYPES 153
...
19.2.2 Exercise: Implement FlipFlopobjects with Counterobjects as their internal
representation type, following the model of the FlipFlopADT in Section 19.2. 2January 15, 2000 19. EXISTENTIAL TYPES 154
Objects vs. ADTs
...
spectrum: when programming with ADTs, packages areopened immediately after
...
anda“continuation,” callsthe continuation to yield afinalresult. The continuationJanuary 15, 2000 19. EXISTENTIAL TYPES 155
takestwo arguments—atype Xanda valueof typeT—anduses them in computing
the final result.
...
canthus supply t(which, by rule T-PACK, hastype fX 7!SgT)asthe next argument:
{9X=S,t} as {9X,T}
def
= Y. f:(8X.T!Y). f [S] t
...
expression.2
As in the Church encodings in Section 18.4, the intuition here is that
...
terms.January 15, 2000 19. EXISTENTIAL TYPES 156
...
correct? 2
19.3.2 Exercise [Recommended]: Takeablankpieceofpaperand,without looking
...
Mitchell and Plotkin [MP88]. (They also noticed the correspondance with objects.)Chapter 20
...
subtyping rules for universal quantifiers areactually refined to take subtyping into
...
(f ra);
I {a=0} : {a:Nat}
157January 15, 2000 20. BOUNDED QUANTIFICATION 158
...
The fact about the operational behavior of f2 that we want to express in its type is:January 15, 2000 20. BOUNDED QUANTIFICATION 159
...
B<:Top; X<:B; Y<:X `B!Y <: X!B: 2January 15, 2000 20. BOUNDED QUANTIFICATION 160
...
t1 t2 !t0
1 t2
(E-APP1)January 15, 2000 20. BOUNDED QUANTIFICATION 161
t2 !t2
0
v1 t2 !v1 t0
2
...
`t : S `S <: T
`t : T
(T-SUB)January 15, 2000 20. BOUNDED QUANTIFICATION 162
Full F<:
...
Moreover, if, for each type U that is an acceptableargument to the functions in both
...
Kernel F<: (or Fk
<:). The same system with the full quantifier subtyping rule is called
Full F<: (or Ff
...
erty? 2January 15, 2000 20. BOUNDED QUANTIFICATION 163
## 20.3 Examples
...
correspond to pairs of T1 and T2. The constructor pairand the destructors fstand
...
i21::n}
def
= pair t1 (pair t2 ... (pair tn top)...);January 15, 2000 20. BOUNDED QUANTIFICATION 164
...
i21::n}
`t.i : Ti
Now, let Lbe a countable set of labels, with a fixed total ordering given by the
bijective function label-with-index : N !L. We define records as follows:
20.3.3 Definition: Let L be a finite subset of Land let Sl be a type for each l 2 L.
Let m be the maximal index of any element of L, and
^ Si =
Sl if label-with-index(i) = l 2L
Top if label-with-index(i) = 2L:
The record type {l:Sl
l2L} is defined as the flexible tuple {^ S
i21::m
i }. Similarly, if tl
is a term for each l : L, then
^ ti =
ti if label-with-index(i) = l 2L
top if label-with-index(i) = 2L:
The record value {l=tl
l2L} is {^ ti21::m
...
i21::n} <: {li:Ti
i21::n}
(S-RCD-DEPTH)January 15, 2000 20. BOUNDED QUANTIFICATION 165
...
Although SZerohasalmost thesameformasSNat,itsayssomething muchstronger
...
function always returns a positive number:January 15, 2000 20. BOUNDED QUANTIFICATION 166
ssucc = n:SNat.
(X. S<:X. Z<:X. s:X!S. z:Z.
s (n [X] [S] [Z] s z))
as SPos;
...
velop in the intervening chapters.January 15, 2000 20. BOUNDED QUANTIFICATION 167
## 20.4 Safety
We now consider the metatheory of both kernel and full systems of bounded quan-
tification (Fk
<:and Ff
<:). Much of the development is the same for both systems: we
...
1. If `t : T and x = 2dom( ),then ;x:S `t : T.
2. If `t : T and X = 2dom( ),then ;X<:S `t : T.
3. If `S <: T and x = 2dom( ),then ;x:S `S <: T.
4. If `S <: T and X = 2dom( ),then ;X<:S `S <: T. 2
...
Next, we have the usual lemma relating substitution and the typing relation.January 15, 2000 20. BOUNDED QUANTIFICATION 168
...
Sincewemaysubstitute typesfortypevariablesduringreduction,wealsoneed
alemmarelatingtypesubstitution andtyping, aswedidinSystemF. Here,though,
...
ward.
20.4.11 Theorem [Preservation]: If `t : T and `t !t0, then `t0 : T. 2
Proof: By induction on a derivation of ` t : T. All of the cases are straightfor-
ward, using the facts established in the above lemmas.January 15, 2000 20. BOUNDED QUANTIFICATION 170
Case T-VAR: t = x
This case cannot actually arise, since we assumed ` t ! t0 and there are no
...
Subcase: t1 is a value `t2 !t0
2 t0 = t1 t0
2
Similar.
Subcase: t1 = x:U11.u12 t0 = fx 7!t2gu12
...
20.4.12 Exercise: Show how to extend the argument in this section to F
f
<:. 2January 15, 2000 20. BOUNDED QUANTIFICATION 171
## 20.5 Bounded Existential Types
This final section remains to be written.
Bounded existentialquantification Fk
<: + 9
New syntactic forms
T ::= ... (types...)
...
Theideaofboundedquantification wasintroducedbyCardelliandWegner[CW85]
in the language Fun. (Their “Kernel Fun” calculus corresponds to our Fk
<:.) Based
oninformalideasbyCardelliandformalizedusing techniquesdevelopedbyMitchell
[Mit84b], FunintegratedGirard-Reynoldspolymorphism [Gir72,Rey74]with Cardelli’s
first-order calculus of subtyping [?, Car84]. The original Fun was simplified and
slightly generalizedbyBruceandLongo [BL90],andagainbyCurienandGhelli[CG92],
...
grammingexamplesusingboundedquantification; morearedevelopedinCardelli’s
study of power kinds [Car88]. Curien and Ghelli [CG92, Ghe90] address a number
of syntactic properties of F<:. Semantic aspectsof closely relatedsystems havebeen
studied by Bruce and Longo [BL90], Martini [Mar88], Breazu-Tannen, Coquand,January 15, 2000 20. BOUNDED QUANTIFICATION 172
Gunter, andScedrov[BCGS91], Cardone[Car89],CardelliandLongo [CL91],Cardelli,
...
` I t1 : T1 ` I T1 *(T11!T12) ` I t2 : T2 ` I T2 <: T11
` I t1 t2 : T12
(TA-APP)January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 175
; X<:T1 ` I t2 : T2
` I X<:T1 .t2 : 8X<:T1 .T2
(TA-TABS)
...
Bytheinductionhypothesis, ;x:T1 ` I t2 : M2 forsomeM2 with ;x:T1 `M2 <: T2,
...
`8X<:T1.M2 <: 8X<:T1.T2.January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 176
...
then this T is also a type for Tin the original typing relation (by part (1) of 21.2.1). If
not, then part (2) of 21.2.1implies that t has no type in the original typing relation.
...
we’d need to guess what type to replace this metavariable with.January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 177
...
rule and replaceit by a restrictedreflexivity rule that appliesonly to type variables.
`X <: X
...
X<:U 2 ` I U <: T
` I X <: T
(SA-TRANS-TVAR)January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 178
` I T1 <: S1 ` I S2 <: T2
` I S1!S2 <: T1!T2
(SA-ARROW)
;X<:U1 ` I S2 <: T2
` I 8X<:U1.S2 <: 8X<:U1.T2
(SA-ALL)
21.3.1 Lemma [Reflexivity of the algorithmic subtype relation]: Thestatement `I
...
1. If `S <: T then ` I S <: T.
2. If ` I S <: T then `S <: T. 2January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 179
...
## 21.3.4 Definition: The weight of a type T in a context , written weight (T), is de-
fined as follows:
weight (X) = weight 1
(U) + 1 if = 1; X<:U; 2
weight (Top) = 1
weight (T1!T2) = weight (T1) + weight (T2) + 1
weight (8X<:T1.T) = weight ;X<:T1
(T2) + 1
...
<:
The only differencein the full system Ff
<: is that the quantifier subtyping rule S-ALL
is replaced by the more expressive variant:
`T1 <: S1 ;X<:T1 `S2 <: T2
`8X<:S1.S2 <: 8X<:T1.T2
(S-ALL)
The algorithmic subtype relation of F
f
<: consists of exactly the same set of rules
...
issues are more subtle.January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 180
...
eachstage ofthe induction, the argumentforpart(2)assumes that part(1)has been
...
ately combine the threesubderivations for the bounds to obtain ` I T1 <: S1.
...
as required.
2. Given Qof a certain size, we use an inner induction on the size of a derivation
...
21.4.2 Exercise: Show thatFk
<: hasjoins andboundedmeets. (Solutionon page261.)
2
## 21.4.3 Exercise: Consider the types (due to Ghelli [Ghe90, p. 92])
S = 8X<:Y!Z.Y!ZJanuary 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 181
and
T = 8X<:Y0
!Z0
.Y0
!Z0
and the context = Y<:Top; Z<:Top; Y0<:Y; Z0<:Z.
1. In Ff
<:, how many types are subtypes of both S and T under .
2. Show that, in F
f
<:, the types S and T have no meet under .
3. Exhibit a pair of types that has no join (under )in F
f
<:.
...
for the original Ff
<: subtyping relation and that terminates on all inputs.
The full proof of this fact is beyond the scope of this book (the argument re-
quiresnoparticularlydeepmathematics, buttakesseveralpagestodevelop). How-
...
 If8X<:T1.T2 is apositive (respectively,negative)occurrence,then T1 is aneg-
...
Proof: Exercise. 2January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 183
...
program.
 Thetype T, the nestednegations, and the bound variablesX0 and Y0 hereplay
...
<: :T by Fact 21.5.4
iff ` T
<: (8Z0<:T.8Z1<:Q.8Z2<:P. U)) by Fact 21.5.2January 15, 2000 21. IMPLEMENTING BOUNDED QUANTIFICATION 184
...
then, on the next cycle of the machine, the currentvalueof register 1(Q) will appear
...
subtyping statement S(M) such that S(M) is derivable in Ff
<: iff the execution of M
halts. 2
...
1. Define a variant of Ff
<: with no Top type but with both X<:T and X bindings.
...
termsequatedbythetheoryhaveequalinterpretations; toshow thatitisconsistent—
...
Inbuildingsemanticmodels ofprogramminglanguages, typesarecommonly mod-
...
[[S]] to [[T]].
For F<:, a more refinedframeworkis required,due principally to the impredica-
...
2For example, a pair (m;n) of numbers can be encoded as 2m  (2  n+ 1).January 15, 2000 22. DENOTATIONAL SEMANTICS 187
...
We canobtain asimple semantics forF<: in this setting by interpretingtypes assub-
...
[[{l1:T1...ln:Tn}]] = ft j 8li 2fl1 :::lng: ftg(li) 2[[Ti]]g:
...
Inordertointerprettypescontaining variables,wemust parameterizetheabove
...
T
U[ [S] ]
[[T]]+fX7!UgJanuary 15, 2000 22. DENOTATIONAL SEMANTICS 188
...
2. If `S<:T then [[S]] [[T]].
3. If `t 2S then [[t]] is an element of [[S]]. 2
...
2. If [[t]]+fx7![ [t0] ]g is defined, then so is [[fx 7!t0gt]] and they are equal. 2January 15, 2000 22. DENOTATIONAL SEMANTICS 189
...
[[fX 7!UgT]]. 2
22.1.4 Exercise [Quick check]: How can it happen that [[fx 7! t0gt]] is defined,
but [[t]]+fx7![ [t0] ]g is not? 2
...
certain types are inhabited by any closed terms. For example, [[8[X<:Top]X]] =
T
UNU is the empty set3
; so there can be no closed term of type 8[X<:Top]X,
...
combinator cannotbeinterpretedinthe subsetmodel, andhencecannot bedefined
in pure System F<:. This ties in with the observation made above that all functions
in the subset model are total.
3From here on, when the environment  in an expression [[T]] is empty, we will usually omit it.January 15, 2000 22. DENOTATIONAL SEMANTICS 190
...
[[t]]+fx7!vg = [[t0]]+fx7!vg for all v 2[[S]]
we cannot conclude that
[[fun[x:S]t]] = [[fun[x:S]t0
]]:
...
represents a failure of extensionalityof the semantics.
...
its own “local”notion of equality. In the caseof function types, this equality will be
...
22.2.1 Definition: A partial equivalencerelation (PER) is a symmetric and transi-
...
viewed at a supertype T).January 15, 2000 22. DENOTATIONAL SEMANTICS 191
...
[[S!T]] = f(t;t0) j 8(x;x0) 2[[S]]: ftg(x)[[T]]ft0g(x0)g
[[{l1:T1...ln:Tn}]] = f(t;t0) j 8li 2fl1 :::lng: (ftgli;ft0gli) 2[[Ti]]g
[[8[X<:S]T]] = f(t;t0) j t[[T]]+fX7!Ugt0 for all U [[S]]g
that is,
T
U[ [S] ]
[[T]]+fX7!Ug
22.2.2 Exercise [Quick check]: Show that, if f;g 2 dom([[S!T]]), then f[[S!T]]g
iff f(a)[[T]]g(a) for all a 2dom([[S]]). 2
...
ing in ,we require that (x) 2dom([[S]]).
22.2.3 Theorem [Semantic soundness of the PER intepretation]: ThePERinterpre-
tation is sound in the sense that all statements are validated semantically. If  j= ,
then:
1. `S <: T implies [[S]] [[T]];
2. `t : S implies [[t]] 2dom([[S]]);
3. t ! t0, with `t : T and `t0 : T, implies [[t]][[T]][[t0]]. 2
...
and (x)[[S]]0(x) for all term variable declarations x:S, then ` t : S
implies [[t]][[S]][[t]]0 .
Notice that [[S]] = [[S]]0 2
...
ing more complicated.January 15, 2000 22. DENOTATIONAL SEMANTICS 192
...
Proof: BythestrongnormalizationofSystemF<: (Theorem??)togetherwithpreser-
...
obs
= S t2. 2January 15, 2000 22. DENOTATIONAL SEMANTICS 193
Proof: Supposet1;t2 : Sand[[t1]][[S]][[t2]]. Ifc : S!Nat, then[[c]] 2dom([[S!Nat]]).
Thus,
...
obs
= t2. 2
The computational adequacytheorem shows how the PER model canbe employed
...
1. Show that if e and f are closed, well typed terms, then e
obs
=Top f.
...
22.2.10 Notation: To reduceclutterin thefollowing, wewill often abbreviateftg(n)
by tn or t(n). 2
## 22.2.11 Proposition: Let S be an arbitrary type and U = [[8[X<:S]X!X]]. If f 2
dom(U), then f U x:x. 2
Proof: We must show that, for every PER X [[S]] and every x 2dom(X), we have
f(x) X x. From f 2 dom(U), we know that, for each PER Y  [[S]] and x 2 dom(Y),
we have f(x) 2dom(Y). Suppose that X [[S]] and x 2dom(X). Choose Y X [[S]]
to be the singleton f(x;x)g. From x 2 dom(Y) we infer f(x) 2 dom(Y), so f(x) = x.
Thus, f(x) X x for every x 2dom(X). 2
...
ond argument. Also, notice that, by computational adequacy, every closed termJanuary 15, 2000 22. DENOTATIONAL SEMANTICS 194
...
Proof: First, recall that f 2 dom(CBool) means that x X x0 and y X y0 imply
f x y X f x0 y0, for every PER X and numbers x;x0;y;y0. In particular, if x;y 2
dom(X), then f x y 2dom(X).
...
0 and 1 replaced by x and y, we find that f x y 2fx;yg. So if x = y, then f x y = x
...
 Supposex = 1andy = 0. LetXbe the PER with equivalence classes f0;2gand
...
f 1 0 2f1;2g. Since f x y 2fx;yg, we conclude that f 1 0 = 1.
The other cases are left as exercises. 2January 15, 2000 22. DENOTATIONAL SEMANTICS 195
...
wereallowedto use arbitraryrelations, not just PERs—if it werethe case,for exam-
ple, that f 2dom(CBool) implied (f x y) R (f x0 y0) whenever x R x0 and y R y0 for
...
PER model.
22.2.13 Proposition: Letf 2[[8[X]X!S]],whereXisnotfreein S. Thenfisconstant—
that is, for each PER X and element x 2dom(X), we have (f x) [[S]] (f 0). 2
...
=V f V (fun[v:V]g(v))(h):
2
Proof: Exercise. 2January 15, 2000 22. DENOTATIONAL SEMANTICS 196
...
t. If S = Nat, then it clearly holds; but for the function space S!Twe encounter the
following problem. Suppose `f;g : S!T and f
obs
= g. To prove [[f]] = [[g]], we have
...
transitive, reflexive, andanti-symmetric ordering v) with the following properties:
1. If x0 vx1 vx2 v:::is an increasing chain in Dthen there exists a least upper
bound
F
i xi 2 D. That is, xi v F
i xi for all i and, if xi v y for all i, then F
i xi vy.
4This can be seen from a simple diagonalization argument. Suppose that (fx)x2Nis an effective
enumeration of all closed F<: terms of type Nat!Nat. Then x: fx(x) + 1is a computable function that
is not definable in F<:.January 15, 2000 22. DENOTATIONAL SEMANTICS 197
...
Dis uniquely determined, in the sense that, if xi vzfor all i and, whenever xi vy
for all i, we have z vy, then z =
F
i xi. 2
...
as the least upper bound of the chain ?vf(?) vf(f(?)) v::: 2
Proof: Exercise. 2
...
Rcd(D) as the set of functions f 2L ! D, ordered pointwise, plus a distinguished
least element ?Rcd(D)
(usually written just ?). Again, this forms a domain.
...
natural numbers plus an extra bottom element ?. The ordering is: x vy iff x = ?
or x = y. 2
...
D  = N? + [D!D] + Rcd(D)January 15, 2000 22. DENOTATIONAL SEMANTICS 198
Thatis, wewill use adomain Dthat “contains”theflat domain ofnaturalnumbers,
the function space [D!D], and the recorddomain Rcd(D). Rather than developing
...
rcd 2 [Rcd(D) !D] rcd?
2 [D !Rcd(D)]
satisfying the following equations (for 2fN;fun;rcdg)
(?(x)) = x
(?) = ?D
?(d) =
e if d = (e) for some e
? otherwise
and such that every element d 2D can be expressed as d = (e) for some e, where
is one of fN;fun;rcdg. Furthermore, there is an increasing sequence of projection
functions
r 2[D !D] for all r 2N
...
Proof: See [Gun92] or any other standard text on domain theory. 2January 15, 2000 22. DENOTATIONAL SEMANTICS 199
...
fun(x 2D1:N(0)); fun(x 2D1:x); and 8 more functions::: g
etc.
## 22.3.7 Exercise:
1. How many elements does D3 contain?
2. Convince yourself that Dr can be described as fr(d) j d 2Dg.
...
with the property that succ(N(n)) = N(n + 1). 2January 15, 2000 22. DENOTATIONAL SEMANTICS 200
...
=
F
i fi(?))will beanelementofdom([[8[X](X!X)!X]]),
werequirethateveryPERusedto interprettypescontains thepair(?;?) and
...
?R ?we see by induction that fi?R fi?for every i. Hence fix(f) 2dom(R).)
...
## 22.3.12 Definition:
1. The CUPER Nat is defined by f(N(n);N(n)) j n 2N?g.
2. The CUPER Top is the total relation D D.
3. When R and S are CUPERs, the CUPER R )S is defined by
f(fun(f);fun(g)) j for all x;y 2D, if x R y then f(x) S g(y)g.
4. IfR1 throughRn areCUPERsandl1 :::ln arelabels, thentheCUPERfl1:R1 :::ln:Rng
isdefinedbyf(?;?)g[f(rcd(f);rcd(g)) jf;g 2L!Dandf(li) Ri g(li) for i = 1:::ng.
5. Let I be any set and (Ri)i2I be an I-indexed family of CUPERs. The CUPER T
...
sub-CUPERs of some CUPER.)January 15, 2000 22. DENOTATIONAL SEMANTICS 201
6. ?is the CUPER f(?;?)g. 2
22.3.13 Exercise [Recommended]: Show thatalloftheseconstructions actuallyde-
fine CUPERs. 2
...
Fk+1(?)k = Fk(?)k for all k 2N (22.1)January 15, 2000 22. DENOTATIONAL SEMANTICS 202
...
NowassumethatxFr(?)ry, hence,x;y 2Dr. FromFk+1(?)k+1 Fk+1(?)k =
...
F((F))r+1 forallr 2N. Now, sinceFiscontractive,F((F))r+1 = F((F)r)r+1 =
F(Fr(?)r)r+1 = Fr+1(?)r+1 = (F)r+1 (using Lemma 22.3.18 in the second
...
ThesubsetandPERmodelsofthesimply typedlambda-calculusareduetoKreisel,
...
{i li=vi
i21::n}.li !vi (E-RCDBETA)
204January 15, 2000 23. POLYMORPHIC UPDATE 205
tj !t0
j
{li=vi
i21::n} !{i li=vi
...
Pair = A: :*. B: :*. 8X. (A!B!X) ! X;
However, since *is by farthe most common case,we allow X.Tas anabbreviation
for X: :*.T.
...
leveloftermscontains actualdatavalues(integers,records,etc.) andcomputations
(functions) over them.
5
x:Nat.x
( x:Nat.x)5
...
the type operator X.X!X. The former classifies terms like X.x:X.x—it is a typeJanuary 15, 2000 24. TYPE OPERATORS AND KINDING 209
...
just thevariables,abstraction,andapplicationofthesimplytypedlambda-calculus;
...
context .”
Second, whenever a type Tappearsin a term (as in x:T.t), we must check that
...
Id Nat ! Bool
Id (Id (Id Nat)) ! Bool
all mean the same thing, in the sense that they have the same terms as members.January 15, 2000 24. TYPE OPERATORS AND KINDING 210
Type operators and kinding !  )
Syntax
t ::= (terms...)
x variable
...
`S U: : K `U T: : K
`S T: : K
(Q-TRANS)January 15, 2000 24. TYPE OPERATORS AND KINDING 211
;X: :K11 `T12 : : K12 `T2 : : K11
`(X: :K11.T12) T2 fX 7!T2gT12 : : K12
(Q-BETA)
`S : : K1)K2 `T : : K1)K2
;X: :K1 `S X T X : : K2
`S T : K1)K2
...
(T-APP)
Abbreviations
unannotated binder for X
def
= X: :*January 15, 2000 24. TYPE OPERATORS AND KINDING 212Chapter 25
...
25.1.1 Exercise: Writeandtypecheckaridiculously polymorphic applyTwicefunc-
...
`t1 [T2] : fX 7!T2gT12
(T-TAPP)January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 215
...
25.2.1 Exercise [Recommended]: WriteaprogramthatdefinesthepairADT,opens
it, and then defines a List ADT with the representation type
List = X. 8R. (X!R!R) ! R ! R;
and with operations nil, cons, car, and cdr of appropriate types. 2January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 216
F! : Summary System F+ )
Syntax
t ::= (terms...)
x variable
...
1 [T2]
(E-TAPP)
Type equivalence ( `S T)January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 217
`T : : K
`T T : : K
(Q-REFL)
`T S : : K
...
x:T 2
`x : T
(T-VAR)January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 218
`T1 : : * ;x:T1 `t2 : T2
`x:T1.t2 : T1!T2
(T-ABS)
`t1 : T11!T12 `t2 : T11
...
`S T : : K, replacing by V and dropping the rule Q-SYMM. 2
25.3.2 Proposition: If `S T : : K, then there is some U such that `S V U : : K
and `T V U : : K. 2
Proof: Standard. 2
## 25.4 Soundness
## 25.4.1 Lemma:
...
Proof: The kinding and type equivalence relations do not dependon term variable
bindings. 2
25.4.2 Lemma [Substitution]: If ; x:S;  `t : T and `s : S, then ;  `fx 7!
sgt : T. 2
Proof: Straightforward. (Quick check: where is Lemma 25.4.1 used?) 2
25.4.3 Theorem [Preservation]: If `t : T and t !t0, then `t0 : T. 2
Proof: Straightforward induction on evaluation derivations, using Lemma 25.4.2
for the T-VAR case. 2January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 219
## 25.4.4 Lemma:
...
`T1!T2 V U. A quick inspection of the rules defining the V relation shows that
...
assumption that it is stuck.January 15, 2000 25. HIGHER-ORDER POLYMORPHISM 220
Case T-TAPP:
Similar. 2Chapter 26
...
` I S1 S2 T1 T2
(QA-TAPP)
221January 15, 2000 26. IMPLEMENTING HIGHER-ORDER SYSTEMS 222
Exposure ( ` I T *T0)
otherwise
` I T *T
(XA-OTHER)
...
Proof: Straightforward induction on typing derivations. 2
26.4 Theorem [Equivalence of declaractive and algorithmic typing]: Supposewe
have ` ok. Then:
1. If ` I t : T, then `t : T.January 15, 2000 26. IMPLEMENTING HIGHER-ORDER SYSTEMS 223
...
F!
<: : Higher-order bounded quantification F<: + F!
Syntax
t ::= (terms...)
x variable
x:T.t abstraction
...
; empty context
;x:T term variable binding
224January 15, 2000 27. HIGHER-ORDER SUBTYPING 225
;X<:T type variable binding
K ::= (kinds...)
* kind of proper types
K)K kind of operators
...
`S1 T1 : : K11)K12 `S2 T2 : : K11
`S1 S2 T1 S2 : : K12
(Q-APP)January 15, 2000 27. HIGHER-ORDER SUBTYPING 226
Kinding ( `T : : K)
X<:T 2 `T : : K
`X : : K
(K-TVAR)
...
`T1 : : * ;x:T1 `t2 : T2
`x:T1.t2 : T1!T2
(T-ABS)January 15, 2000 27. HIGHER-ORDER SUBTYPING 227
`t1 : T11!T12 `t2 : T11
`t1 t2 : T12
(T-APP)
`t : S `S <: T
...
TopK1)K2
def
= X: :K1. TopK2Chapter 28
Pure Objects and Classes
Technically complete, more or less. Work needed on writing.
## 28.1 Simple Objects
Recall (from Chapter 19) the type of pure (or “purely functional”) Counterobjects:
Counter = {9X, {state:X, methods:{get: X!Nat, inc:X!X}}};
...
methods = body.methods}}
as Counter;
228January 15, 2000 28. PURE OBJECTS AND CLASSES 229
I sendinc : Counter ! Counter
addthree = c:Counter. sendinc (sendinc (sendinc c));
I addthree : Counter ! Counter
c3 = addthree c;
...
ThismeansthatifwedefineanobjecttypewithmoremethodsthanourCounter
type
ResetCounter =
...
as ResetCounter;
I rc : ResetCounter
wecanpassitasalegalargumentto sendgetandsendinc,andhencealsoaddthree:
rc3 = addthree rc;
sendget rc3;
I rc3 : Counter
3 : Nat
Notice, though, that the type of rc3 here is just Counter – passing it through the
addthreeoperation has lost some information about its type.January 15, 2000 28. PURE OBJECTS AND CLASSES 230
...
We can now give sendinca more refined typing by abstracting over sub-operators
of CounterM
sendinc =
M<:CounterM.
o:Object(M).
...
methods = b.methods}}
as Object(M));January 15, 2000 28. PURE OBJECTS AND CLASSES 231
I sendinc : 8M<:CounterM. Object M ! Object M
andsendmessagestocounterandreset-counterobjects likethis(assumingsendget
and sendresetare defined analogously):
...
28.4.2 Exercise: Define sendgetand sendreset. 2
## 28.5 Simple Classes
...
Or, using the CounterMoperator to write the annotation more tersely:
counterClass =
{get = x:CounterR. x,
inc = x:CounterR. succ(x)}
as CounterM CounterR;January 15, 2000 28. PURE OBJECTS AND CLASSES 232
...
exactly type CounterR: we can also work with states that are subtypes of this type.January 15, 2000 28. PURE OBJECTS AND CLASSES 233
counterClass =
R<:CounterR.
{get = s:R. s.x,
...
as Counter;
I c : Counter
We can write resetCounterClassin the same style:
resetCounterClass =
R<:CounterR.
let super = counterClass [R] in
{get = super.get,
...
record update. 2January 15, 2000 28. PURE OBJECTS AND CLASSES 234
...
We begin by abstracting counterClasson a collection of methods (appropriate
...
instrumentedCounterClass =January 15, 2000 28. PURE OBJECTS AND CLASSES 235
...
terms of self.set.
## 28.8 Class Types
We can write the type of counterClassas
...
{state = r,
methods = fix (c [R])}}
as Object M;January 15, 2000 28. PURE OBJECTS AND CLASSES 236
I new : 8M<:TopM. 8R. Class M R ! R ! Object M
where we write TopM for the “maximal interface” X.Top:
TopM = X.Top;
We can now build counter objects by applying new to counterClassas follows:
...
Class SelfM SelfR)January 15, 2000 28. PURE OBJECTS AND CLASSES 237
...
eventually be providedby the newfunction) and avector of “self methods” special-
...
Now we can recover setCounterClassby extending counterClasslike this:
setCounterClass =
...
28.9.1 Exercise [Recommended]: Use the fullupdatechecker from the course di-
rectory to implement the following extensions to the classes above:
...
x:T.t abstraction
t t application
{SX=T: :K,t} structure creation
bodyof(t) term component of a structure
v ::= (values...)
x:T.t abstraction value
{SX=T: :K,v} structure value
T ::= (types...)
X type variable
T!T type of functions
{SX: :K,T} structure type
...
(Q-ARROW)
`T1 : : K1 `fX 7!T1gt2 : T2
`typeof({SX=T1,t2}) T1 : : K1
(Q-STRUCTBETA)January 15, 2000 29. STRUCTURES AND MODULES 240
Kinding ( `T : : K)
`T1 : : * `T2 : : *
`T1!T2 : : *
(K-ARROW)
...
immediately ascribed, then the term will have a principal type.January 15, 2000 29. STRUCTURES AND MODULES 242
...
i21::n} ok
(KF-TRCD)
New kind equivalence rulesJanuary 15, 2000 29. STRUCTURES AND MODULES 243
 is a permutation of f1::ng
`{li: :Ki
i21::n} ok
`{li: :Ki21::n
...
– The present algorithm relies on it (weak argument, but there it is)January 15, 2000 29. STRUCTURES AND MODULES 244
...
`S : : *
`S : : Eq(S)
(K-SINGLETON)January 15, 2000 29. STRUCTURES AND MODULES 245
...
`T0 : : {li .Xi : :Ki
i21::n}
`T0.lj : : fX1 7!T0.l1gfXj-1 7!T0.lj-1gKj
(K-TYPROJ)
New subkinding rules
`{li .Xi : :Ki
i21::n+k} ok
...
i21::n+k} <: {li .Xi : :Ki
i21::n}
(S-TRCD-WIDTH)January 15, 2000 29. STRUCTURES AND MODULES 246
`{li .Xi : :Ji
i21::n} ok
`{li .Xi : :Ki
i21::n} ok
;X1: :K1 Xi-1: :Ki-1 ` i21::n
Si <: Ti
`{li .Xi : :Ji
i21::n} <: {li .Xi : :Ki
i21::n}
...
doesn’twork in the presenceofdependencies,becauseit presumesthe ability
to reorder labels arbitrarily to match the fixed global order; but this may
break dependencies between fields.
29.5 DependentRecordExpressionsandRecordsofTypes
...
for some j < i, xj 2FV(ti)
t0
i = fx1 7!v1gfxi-1 7!vi-1gti
{lj .xj =vj
...
i21::n} : {li .xi :Ti
i21::n}
(T-RCD)January 15, 2000 29. STRUCTURES AND MODULES 247
New abbreviations
{...ti...}
def
...
`T : : X: :K1.K0
2
(K-TARR-SELF)January 15, 2000 29. STRUCTURES AND MODULES 248
`T : : {li.Xi: :Ki
i21::n}
;X1: :K1 Xi-1: :Ki-1 `i21::n T.li : : K0
i
`T : : {li.Xi: :K0
i
i21::n}
...
`x:T1.t2 : x:T1.T2
(T-ABS)
`t1 : x:T11.T12 x = 2FV(T11) `t2 : T11
`t1 t2 : T12
(T-APP)
New abbreviations
T1!T2
def
= x:T1.T2 where x = 2FV(T2)January 15, 2000 29. STRUCTURES AND MODULES 249
Dependent records ????
New syntactic forms
T ::= ... (types...)
{li .xi :Ti
...
these provide the mechanism for propagating sharingJanuary 15, 2000 29. STRUCTURES AND MODULES 250
...
theusualissues incombination witheffects). This version oftherule, though,
...
the term the type
{l.x:{SEq(Int),...}, l'.x':typeof(x)}
(asubtype of the previous type), fromwhich T-RCD does derivethe intended
type.
...
substitution, which doesn’tscaletosystemswith effects(theHarper/Lillibridge
calculuscanbedescribedasavariantofstrongsums thatdoesthis right)
...
be used to derive t ! t0 and t ! t00 (all the other rules have left-hand sides
whose outermost constructor is something other than succ). So there must be
two subderivations with conclusions of the form t1 ! t0
1 and t1 ! t00
1. By
...
1, as required.
Case: t = pred t1
Here there arethree evaluation rules (E-PRED, E-BETANATIZ, and E-BETANATIS)
that might have been used to reduct t to t0 and t00. Notice, however, that these
rules do not overlap: if t matches the left-hand side of one rule, then it definitely
does not match the left-hand side of the others. (For example, if tmatches E-PRED,
then t1 is definitely not a value, in particular not 0 or succ v.) This tells us that
the same rule must have been used to derive t !t0 and t !t00. If that rule was
...
equal (sumlist l) c9;
I (x. y. x)January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 255
...
a; b; c; d, then (c) = 1 and nc = a; b; d.
Now, the property that we want is that, if x = 2FV(s), then
...
Solutionto6.3.8: Here’sacounterexample: theterm(if false then true else 0)
...
T-APP, `fx 7!sgt1 fx 7!sgt2 : T, i.e., `fx 7!sg(t1 t2) : T.January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 256
...
`F x : T jF fg
(CT-VAR)
;x:S `F t1 : T jF0 C x 62dom( )
`F x:S.t1 : S!T jF0 C
(CT-ABS)
`F t1 : T1 jF0 C1 `F0 t2 : T2 jF00 C2 F00 = X; F000
`F t1 t2 : X jF000 C1 [C2 [fT1 = T2!Xg
(CT-APP)
`F 0 : Nat jF fg (CT-ZERO)
`F t1 : T jF0 C
`F succ t1 : Nat jF0 C [fT = Natg
(CT-SUCC)January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 257
`F t1 : T1 jF0 C1 `F0 t2 : T2 jF00 C2 `F00 t3 : T3 jF000 C3
C0 = C1 [C2 [C3 [fT1 = Nat; T2 = T3!T3g
`F iter T t1 t2 t3 : T3 jF000 C0 (CT-ITER)
Theequivalenceof theoriginal rules andthealgorithmic presentationcanbestated
as follows:
1. (Soundness) If `F t : T jF0 C and the variables mentioned in and t do
not appear in F, then `t : T jF0nF C.
...
`F t : T jF0 C, then the type variables mentioned in T and C do not
appear in F0 nF.
For the same case in part 2, the following lemma is used:
If `F t : T jF0 C, then `F;G t : T jF0;G C, where G is any sequence
...
| TmSucc(fi,t1) !
let (tyT1,nextuvar1,constr1) = recon ctx nextuvar t1 in
(TyNat, nextuvar1, [(tyT1,TyNat)])January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 258
| TmPred(fi,t1) !
let (tyT1,nextuvar1,constr1) = recon ctx nextuvar t1 in
...
| TyId(s) ! (s=tyX)
in o tyT
Theunification functionisnow adirecttranscriptionoftherulesgivenonpage126.
...
for a language with type reconstruction.)January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 259
...
(CT-PROJ)January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 260
...
thesimplesets ofequations betweentypeswith unification variablesoftheoriginal
...
Untyped lambda-calculus:January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 261
D = X. X!X;
lam = f:D!D. fold [D] f;
...
simultaneously as follows. (Note that some of the cases of the definition overlap.January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 262
...
needed to calculate M or J). 2January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 263
A.2 Proposition:
...
`V _ W = J
`P ^ Q = N:January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 264
...
Case: U = 8X<:A.B
Similar. 2January 15, 2000 A. SOLUTIONS TO SELECTED EXERCISES 265
Solution to 21.4.3:
...
8X<:A0
!B. X
arelowerboundsforSandT, butthesetwotypeshavenocommon supertype
that is also a subtype of S and T.
3. ConsiderS!TopandT!Top. (Or,ifyoulike, 8X<:A0!B. A!B0 and8X<:A0!B. X.)
Solutionto??: Notethat the two kinds of quantifiers—bounded andunbounded—
...
T- typing
TA- algorithmic typingAppendix C
...
Scheme, Modula 3, or even Java.1
All of these projects can be done in Pascal, C, or
...
cles[AC94b, AC94a, etc.]),AbadiandCardellihaveproposeda primitive cal-
...
programming in F<:. Using the everythingtypechecker from the course web
directory, develop a more significant object-oriented program following the
same lines.
(a) To get started,implement agroup of collectionclasses (forexample, using
...
ning’s logic programming language ELF [Pfe89, Pfe91, Pfe94, Pfe96].January 15, 2000 C. SUGGESTIONS FOR LARGER PROJECTS 270
(a) Download and install the ELF system.
(b) Useittoencodethesyntaxandtyping rulesofthesimply typedlambda-
calculus with subtyping andprovesome simple theorems aboutthe sys-
...
TWM95].
3. [I]Implementatypedassembly languageinthestyleofMorrisettandco-workers[MWCG98,
MCGW98, MCG+
99].
4. [T]Agreatvarietyoftype systems forextensible records, developedby various
...
techniquesto buildanMLcompilerthatrunswithout agarbagecollector[TT97,
...
theoretic tools of existential types and (a limited form of) dependent types.January 15, 2000 C. SUGGESTIONS FOR LARGER PROJECTS 271
Theseaccountsplacepowerfulmodulesystems likethoseofStandardML[MTH90]
and Objective Caml [Ler95] on a well-understood and tractable theoretical
foundation. Read and implement one of these papers.
7. [I]Aseries ofpapers[ACPP91,LM91,ACPR95]haveproposedaddingatype
Dynamicto statically typed languages, in order to provide a smooth interface
between statically typed and dynamically typed data.
(a) AddDynamictotheimplementation ofthesimplytypedlambda-calculus,
following [ACPP91].
...
ware(TACS),LogicinComputerScience(LICS),(amongmanyothers). Leafingthrough
...
tion of the system it describes.Appendix D
...
not alreadyfamiliarwith it. Forprogrammersalreadyfamiliarwith another dialect
...
You’ll findtheprogrammingexercisesandexperimentsthroughout thenotesmuch
...
import "filename"; include filenameat this point
...
(where<checkername>isthenameofthecheckerthatyouwant—untyped, fulluntyped,
...
obtained from ftp://ftp.inria.fr/lang. It’s quite easy to install.Bibliography
...
January 1995. Preliminaryversion in Proceedingsof the ACM SIGPLAN Work-
...
[BDCd95] Franco Barbanera, MariangiolaDezani-Ciancaglini, and Ugode’Liguoro.Inter-
...
[Ben95] Johan Van Benthem. Language inAction: Categories, Lambdas, and Dynamic Logic.
MIT Press, 1995.
...
tion and Computation 76(2/3):138–164, 1988.January 15, 2000 BIBLIOGRAPHY 277
...
[Car90] Luca Cardelli. Notes about F!
...
and John C. Mitchell, editors,TheoreticalAspects of Object-OrientedProgramming:
...
[CG92] Pierre-LouisCurien and GiorgioGhelli. Coherence of subsumption: Minimum
...
(MIT Press, 1994).January 15, 2000 BIBLIOGRAPHY 278
...
[Chu40] Alonzo Church. A formulation of the simpletheory of types. Journal of Symbolic
Logic, 5:56–68, 1940.
[Chu41] Alonzo Church. The Calculi of Lambda Conversion. Princeton University Press,
1941.
[CL91] Luca Cardelliand GiuseppeLongo. A semantic basis for Quest. Journal of Func-
...
tension of systemF with subtyping. Informationand Computation, 109(1–2):4–56,
...
and polymorphism. Computing Surveys, 17(4):471–522, December 1985.January 15, 2000 BIBLIOGRAPHY 279
[dB72] Nicolas G. de Bruijn. Lambda-calculus notation with nameless dummies: a
toolforautomatic formulamanipulation with application tothe Church-Rosser
...
206-214.
[DM82] LuisDamas and Robin Milner. Principal typeschemes forfunctional programs.
...
Categories inComputerScienceandLogic, volume92 ofContemporary Mathematics,
...
mentation. Addison-Wesley, Reading, MA, 1983.January 15, 2000 BIBLIOGRAPHY 280
...
[HL94] Robert Harperand MarkLillibridge.A type-theoreticapproachto higher-order
...
[How80] W. A. Howard. The formulas-as-typesnotion of construction. In J.P. Seldinand
...
pages186–197. ACM,January 1995. Fullversionin Informationand Computation,
...
fects. In Conference Record of the Eighteenth Annual ACM Symposium on PrinciplesJanuary 15, 2000 BIBLIOGRAPHY 281
...
rithms forPureTypeSystems. In Henk Barendregtand TobiasNipkow, editors,
...
cidable. In Proceedingsof the ACMSIGPLANWorkshop onMLand itsApplications,
June 1992.
...
[Lan66] P. J. Landin. The next 700 programminglanguages. Communicationsof the ACM,
...
Computer Science, University of Edinburgh, June 1990.January 15, 2000 BIBLIOGRAPHY 282
[LW91] X.Leroyand P.Weis. Polymorphictypeinferenceand assignment. InEighteenth
...
1982.
[Mar88] Simone Martini. Boundedquantifiers have interval models. In Proceedings of the
...
[Mil91] Robin Milner. The polyadic-calculus: a tutorial. Technical ReportECS–LFCS–
...
bridge, MA, 1996.January 15, 2000 BIBLIOGRAPHY 283
...
ming, pages 299–344, Stockholm, Sweden, January 1991. Springer-VerlagLNAI
596.
...
361–375, Utrecht, The Netherlands, March 1993. Springer-Verlag. TLCA’93.January 15, 2000 BIBLIOGRAPHY 284
...
abstract.
[Pfe96] Frank Pfenning. The practice of logicalframeworks. In Hélène Kirchner, editor,
Proceedings of the Colloquiumon Trees inAlgebra and Programming, pages 119–134,
...
pages 561–577. Academic Press, New York, 1980.January 15, 2000 BIBLIOGRAPHY 285
...
[Pot98] François Pottier. A frameworkfortypeinference with subtyping. In Proceedings
of the third ACM SIGPLAN International Conference on Functional Programming
(ICFP ’98), pages 228–238, September 1998.
[PS94] Jens Palsbergand MichaelI. Schwartzbach. Object-Oriented Type Systems. Wiley,
1994.
...
[Rey84] J. C. Reynolds. Polymorphismis not set-theoretic. In G. Kahn, D.B. MacQueen,
...
ence, pages 364–371, Paris, France, 4–7 July 1994. IEEE Computer Society Press.January 15, 2000 BIBLIOGRAPHY 286
...
Projet Formel, INRIA-ENS, 1989.January 15, 2000 BIBLIOGRAPHY 287
...
ence, pages 473–491. Springer-Verlag, New York, N.Y., 1992.Index
Ff
<:(“Full” bounded quantification), 162
Fk
<:(Bounded quantification), 160
F!
<: (Higher-orderboundedquantification),
224
F!(Higher-orderpolymorphiclambda-calculus),
214
...
<:(Simplytypedlambda-calculuswith sub-
typing), 89
F!(Summary), 216
...
KF-HOS, 247January 15, 2000 INDEX 290
KF-SINGLETON, 244
KF-TARR, 245
KF-TRCD, 242, 245
M-RCD, 71
...
T-LRCD, 75
T-NIL, 74
T-NULL, 74January 15, 2000 INDEX 291
...
Type systems and components
Ff
<:: “Full”boundedquantification
[! 8 <: bq full], 162
Algorithmic rules for equi-recursive
types [! B ], 113
Algorithmicrulesforrecordswith per-
mutation [! fg ], 82
Algorithmic rules for simply typed
lambda-calculus [! B], 82
Algorithmicrulesfortypedefinitions
[! B $], 84
...
Boundedexistentialquantification [Fk
<:+
9], 171
Type systems and components
Fk
...
Type systems and components
F!
<: : Higher-orderboundedquan-
tification [F<:+ F!], 224
Higher-order existential types [F !
<: +
9], 215
Type systems and components
F!: Higher-orderpolymorphiclambda-
...
Lazy records [! fg lazy], 75January 15, 2000 INDEX 292
Let binding [! let], 68
Lists [! B List], 73
Type systems and components
SystemF:Polymorphiclambda-calculus
[!8], 133
Polymorphicupdate[F<:+ fg update],
204
...
90
Type systems and components
F!: Summary[SystemF+)], 216
Type definitions [!B $], 83
Type operators and kinding [! 
)], 210
Typedrecordpatterns [!fg let pat(typed)],
256
Typingrulesforbooleans[B (typed)],
56
Typingrulesfornumbers[B N (typed)],
56
Unit type [! Unit], 67
Type systems and components
: Untyped lambda-calculus [!
